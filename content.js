...
const fill = (time) => {
  missings.forEach((missing) => {
    ...
    missing.querySelector('[aria-describedby="NameTrnFnc_startAW"] input[name="Time_startAW"]').value = `${time.start}:00`;
    ...
    missing.querySelector('[aria-describedby="NameTrnFnc_endAW"] input[name="Time_endAW"]').value = time.end;
...
chrome.runtime.onMessage.addListener(({ message, time }, sender, sendResponse) => {
  if (message === "fill") {
    fill(time);
  }
